...
urlpatterns, json_urls, html_urls, socket_urls = [], [], [], []

if "apps.base" in settings.INSTALLED_APPS:
    json_urls.append(path("", include("apps.base.json_urls"), name="base-json"))
    html_urls.append(path("", include("apps.base.html_urls"), name="base-html"))

if "infinite_redis" in settings.INSTALLED_APPS:
    json_urls.append(path("", include("apps.infinite_redis.urls")))

urlpatterns.extend(json_urls)
...
urlpatterns.extend([*html_urls, *socket_urls])
urlpatterns.append(path("infinite_admin/", admin.site.urls))

if "admin_honeypot" in settings.INSTALLED_APPS:
    urlpatterns.append(path("admin/", include("admin_honeypot.urls")))
